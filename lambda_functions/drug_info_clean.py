...
def remove_null_values(event, context):
    try:
        ...
        df2=df[['primaryid','caseid','drug_seq','role_cod','drugname','prod_ai','val_vbm','route','dose_form','dose_amt','serialid']]
        df2=df2.dropna()
        df2=df2.drop_duplicates()
        df2['caseid'].astype(int)
        # Convert the resulting DataFrame to a CSV string
        cleaned_csv = df2.to_csv(index=False)
